...
const SlidingDiv = (props) => {
  ...
  const failed_task_dep_type = (failed_task_instance?.task_type == "DEPLOY" || failed_task_instance?.task_type == "GLOBAL_DEPLOY") ? failed_task_definition?.deployment_type == "CANARY" ? "canary" : "rolling" : null;
  ...
  const postContinuePipelineData = () => {
    ...
    var post_data = {}
    if ((find_task_type == "DEPLOY" || find_task_type == "GLOBAL_DEPLOY") && failed_task_dep_type == "canary") {
      post_data = {
        "stage_instance_id": failed_stage_instance && failed_stage_instance.id,
        "task_instance_id": failed_task_instance && failed_task_instance.id,
