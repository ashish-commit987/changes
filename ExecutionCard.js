...
const ExecutionCard = ({ startPollingAgain, ...props }) => {
    ...
    useEffect(() => {
        setState(prev => ({ ...prev, show_approval_form: true, show_resume_form: true }));
    }, []);
    ...
    function handleSuccessContinuePipelineFailure(data) {
        ...
        showSnackbar('success', success_msg);
    ...
    function handleContinueFailed(error) {
        ...
        showSnackbar('error', error_msg_obj);
    }



    function postApprovalRequest(data, stage_instance_id) {
        if (!pipeline_instance?.pipeline?.id || !pipeline_instance?.id) {
            handleFailedApproved("Pipeline Instance ID missing - cannot approve.");
            return;
        }
        var url = GenerateURL({
            pipeline_id: pipeline_instance.pipeline.id,
        ...
        // Do not instantly hide immediately; let the success/fail handle hiding the form once API finishes.
    }

    function postResumeRequest() {
        if (!pipeline_instance?.pipeline?.id || !pipeline_instance?.id) {
            handleFailedResume("Pipeline Instance ID missing - cannot resume.");
            return;
        }
        var url = GenerateURL({
            pipeline_id: pipeline_instance.pipeline.id,
        ...
        try {
            ...
            setState((new_state) => ({ ...new_state, resume_in_progress: true }));
            PostData(url, data, handleSuccesResume, handleFailedResume);
        } catch (error) {
            setState({
                ...state,
                error: {
    ...
    function handleSuccesResume(data) {
        setState((new_state) => ({
            ...new_state,
        ...
        <>
            <MainDiv>
                ...
                <div className='ml-auto d-flex'>
                    ...
                    {/* <button onClick={toggleFullScreen} className="btn-icon-square bg-purple-text-white"><span className="ri-fullscreen-fill"></span></button> */}
                    {
                ...
                state.show_approval_form ?
                    <ApprovalDiv>
                        ...
                        <div className="btn-section">
                            ...
                            <Button className="btn-blue" isLoading={state.approval_in_progress} onClick={postApprovalRequest} style={{ border: 'none' }}>Submit</Button>
                        </div>
                    </ApprovalDiv> : null
            }
            {
                state.show_resume_form ?
                    <ApprovalDiv>
                        ...
                        <div className="btn-section">
                            ...
                            <Button className="btn-blue" isLoading={state.resume_in_progress} onClick={postResumeRequest} style={{ border: 'none' }}>Submit</Button>
                        </div>
                    </ApprovalDiv> : null
            }
            <MainCard>
    ...
    return (
        <>
            <ExecutionView id="viewFullScreenCanvas">

                {
                    Object.keys(stage_object_with_ids).map((key, index) => (
                        <>
                            {
                                index != 0 ?
                                    <div className="circular-arrow"><span className="ri-arrow-right-fill"></span></div>
                                    : null
                            }
                            <div>
                                <div
                ...
                <button onClick={toggleFullScreen} style={{ position: 'fixed', bottom: '2rem', right: '1rem' }} className="btn-icon-square bg-purple-text-white exeview"><span className="ri-fullscreen-exit-fill"></span></button>
            ...
            {/* <button onClick={toggleFullScreen} className="btn-icon-square bg-purple-text-white" style={{ position: 'absolute', bottom: '1rem', right: '1rem' }}><span className="ri-fullscreen-fill"></span></button> */}
            <SlidingDiv
                data={pipeline_instance.stage_instance}
            ...
            >
                <MenuItem style={{ display: "grid" }} onClick={onViewParameterClick}>
                    <span className='font-12 color-secondary font-weight-600 cursor-pointer'>
                        {/* <span className="ri-google-drive-fill" style={{color:"#505050",marginLeft:'0px'}} ></span> */}
                        View Run Parameters
                    </span>
                </MenuItem>
